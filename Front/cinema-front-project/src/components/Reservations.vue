<template>
    <div v-if="props.reservation.ticket_set.length > 0" id="reservation">
        ...
        <div id="reservation-detail">
            <p>극장 위치 : {{ region }}/{{ props.reservation.ticket_set[0].theater.name }}</p>
            <p>상영 날짜 : {{ props.reservation.ticket_set[0].time.date }}</p>
            <p>상영 시간 : {{ props.reservation.ticket_set[0].time.time }}</p>

            <div id="reservation-detail-seats" v-for="ticket in props.reservation.ticket_set" :key="ticket.id">
...
onMounted(()=>{
    for (const obj in props.regions) {
        if (props.regions[obj].id === props.reservation.ticket_set[0]?.theater.regions){
